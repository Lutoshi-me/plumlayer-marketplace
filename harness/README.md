...
| 12 | Plan inventory | The shipped `scripts/plan_inventory.py`, imported in-process and run end to end over an invented fixture set: a 31-sheet grid across six disciplines (one with no discipline, one with no sheet type, every vocabulary type present, one trade whose sheets exceed a pass), a packages fixture, a kinds fixture, and an index fixture with and without located codes. Asserts the inventory mode's counts against a tally the check computes itself; that `--expect-count` off by one refuses in one line; that window 1 selects exactly the vocabulary sheets plus includes minus excludes, once each, every unit line citing the page the grid gives it; that window 2 writes one pass per package, splits the over-twelve trade into balanced lettered passes, runs each seam group of trades as one contiguous block and names a group no line order can satisfy, resolves a package's code to its trade file by nearest CSI ancestor and counts those, and reports packages with no sheet family and families that named no sheet; that window 3 is exactly the inventory minus window 2's own unit lines and reports the index's open entries; that an index with no located codes is reported as a partial input; and that ten broken invocations each refuse with exit 1 and one stderr line naming what is missing. Every window's bounds line must name its counts. Honest bound: the fixtures are invented and small, so this proves the script's arithmetic and its refusals, and nothing about how a real grid or index file's fields arrive. |
