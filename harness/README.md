...
| 5 | Estimator-block identical | The `## Talk to your user like an estimator` section is byte-identical across every skill that carries it. On a mismatch, names the differing files and the first differing line. |
| 6 | Banned strings | No client-name denylist hit, `PLU-\d+`, internal vault filename, `MOSOT`, em dash, or middle dot in shipped text. The full set applies to skills, the root README, and all four manifest JSON files; the client-name denylist alone applies to `trade-packages/`. Each hit is reported with file, line, and the offending match. |
