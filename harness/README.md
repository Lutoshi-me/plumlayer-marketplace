...
| 11 | Plan inventory | The shipped `scripts/plan_inventory.py`, imported in-process and run end to end over an invented fixture set: a 39-sheet grid across seven disciplines (one with no discipline, one with no sheet type, every vocabulary type present, every window 2 type present in an order that is not the reading order, one discipline whose sheets exceed a pass) and two packages fixtures. Asserts the inventory mode's counts and its unit keys against a tally the check computes itself; that `--expect-count` off by one refuses in one line; that window 1 selects exactly the vocabulary sheets plus includes minus excludes, once each, every unit line citing the page the grid gives it, and writes a `plan/window-1.json` matching that selection key for key; that window 2 is an exact partition of the inventory minus both of window 1's lists, grouped by discipline in inventory order, sorted inside a discipline by the pinned sheet type order with inventory order kept inside a type, and split into balanced lettered passes at twelve; that both of the script's sheet type constants equal the lists pinned in the check and name no type the recognizer does not produce; that window 3 plans one review per package over both packages fixtures, in package order, with two packages on one trade both planning and planned one after the other; that every window 3 unit id is a legal `verify_unit` subject prefix stem with `scopeItem:` in front and `-` behind and none a prefix of another, which is the join the plan and the reviewer would otherwise first meet on mid-run; that a first plan of windows 1 and 2 numbers each pass's units from 1, so the change to unit ids is inert where neither tier has anything to say, and that a replan keeps every id the run ledger's dispatch lines and the script's own previous plan file already handed out, numbers new sheets after the highest number that pass has ever carried, never hands a cut sheet's id out again, lets the ledger win a disagreement with the plan file, reads an id it wrote back off a dispatch line onto the same sheet, and orders a pass by id rather than by the grid, with window 2 placing its new sheets in the pinned type order among themselves; that a window 2 slice planned with `--only` holds exactly the sheets its patterns name and defers every other sheet window 1 left, with its bounds line (`units N plus deferred M equals distinct sheets N+M`, and `sheets deferred M by --only`), its `plan/window-2.json` and its read plan's per-pattern and deferred sections each matching an independent tally, and that the slice is lettered by the whole window, so the full plan after it keeps the slice's ids at the front of part a (three dispatched and the rest planned, then twelve dispatched) and states its partition exactly as before once nothing is deferred; that every pass block counts `units verified` off the run ledger's `verified ... result ok` lines and marks exactly those unit lines, with a `mismatch` line, an id no plan carries and another window's line marking nothing, which is what lets a resume finish a pass unit by unit rather than by its `pass:` line; that a planned and never dispatched id whose sheet now plans in another part is numbered in that part with its old id retired, while a dispatched one is still refused; that a second slice holds the first one's dispatched ids rather than counting them retired; that `plan/window-1.json` carries the include and exclude patterns it was planned with, so a resume can run it again exactly; and that twenty-one broken invocations each refuse with exit 1 and one stderr line naming what is wrong, among them a window 1 file naming a key as both selected and excluded, which corrupts the count the bounds line says out loud while still leaving window 2 a correct partition, and a grid whose two rows fold to one unit key, the case where a sheet would otherwise vanish from every window with nothing said about it, since window 2 subtracts by that string and neither colliding row ever reaches the remainder its partition check counts. Every window's bounds line must name its counts. Seven of them are about a unit id already handed out: a ledger dispatch line whose unit id is not `<pass>-<number>`, one sheet number under two unit ids, one unit id over two sheet sets, a bound sheet number the inventory holds twice, a bound unit whose sheets are only partly still in the cut, a bound unit whose sheets all stay in the cut and now plan as two units, and a dispatched unit whose sheet now plans under a different pass part. Every one of them would move an id the record already carries rows under. Three more are the slice's: an `--only` on window 1, an `--only` matching no sheet window 2 reads, and a slice of thirteen dispatched in one discipline, which the full plan's first part of twelve cannot hold, pinning the limit a slice lives within rather than lifting it. Honest bound: the fixtures are invented and small. They carry the field names the shipped verbs return, so a rename on the record's side fails here, but nothing about how a real grid or a real packages read arrives is proved by them. |
| 12 | Pass summary | The shipped `scripts/pass_summary.py`, imported in-process and run over two invented ledger fixtures. A pass runner verifies each unit against the record and appends that unit's counts to the run ledger as it goes; the script sums those lines back into the pass summary shape so the runner writes what it returns rather than adding the counts a second time. Asserts the units read, every per unit line, the totals, the conflicting rows, the definitions kinds union and every note line against a tally the check computes itself off the same fixture text; that the labels the script prints are exactly the labels of the skill's own published summary block, in order, which is the join the script and the shape would otherwise first meet on mid-run; that another pass, another window and the lead's own `pass:` line reach no total; that a unit dispatched with no verified line prints its counts as a dash, is left out of the totals and is named on the bounds line; that a `result mismatch` unit prints as unverified; that the ledger line names this pass's last line rather than the file's; that the three note kinds the shape carries no line for are counted on the bounds line rather than dropped in silence; that the trades line is the sum of the pass's trades file, ordered by item count then catalog id, and says plainly that there is none where none is given; and that ten broken invocations each refuse with exit 1, one stderr line naming what is wrong, and nothing on stdout. stdout carries the summary and nothing else, and the bounds line goes to stderr beside it, because stdout is what the runner writes. Honest bound: the fixtures are ledger text, so this proves the script's arithmetic, where each note lands and what it refuses, and nothing about a real verification. |
| 13 | No fork subagent | No shipped skill or agent file names `fork` as a subagent type, in either the `subagent_type:` dispatch-line shape this codebase's own templates use, or a `tools: Agent(fork)` frontmatter declaration. Regression guard for a runner that invented a wait primitive by dispatching fork agents (one told to wait for a reader's completion, two told to do nothing and return done) instead of making the reader dispatch itself the wait. Ordinary English uses of "fork" (a forklift, a decision fork) are untouched: the pattern only matches "fork" sitting immediately after one of the two anchors. |
| 14 | Agent tool surface | No `agents/*.md` tools line takes the whole connector as a wildcard; every connector verb an agent's body names (inline code spans, joined across line wraps, plus a `verb(` call shape inside a fenced block) is on that agent's tools line; every declared connector verb is one of the 113 the connector registers, pinned here as a copy of the api's own registrations on the `LEDGER_NOTE_KINDS` precedent; and the prohibition-only exception table stays honest, each entry still named in that agent's text and still off its tools line. Spans are found over the joined body because one wraps mid-span in the reviewer, and a per-line scan desynchronizes on the unclosed backtick and reads prose as code from there on. Runs its own helper over three agent fixtures, so a clean definition is shown to pass and a wildcard line and a named-but-undeclared verb each to refuse in one line naming what is wrong. Cannot judge whether a declared verb is one that agent should hold; that stays in review. |
| 15 | Agent model pinned | Every `agents/*.md` frontmatter carries a non-empty `model`, so a dispatched agent does not run on whatever model the session happens to be on. Reports each agent's pinned model. Runs its own helper over the clean fixture and a fixture with no `model`, so the refusal is shown rather than assumed. Which model a definition names stays a decision, not a check. |
| 16 | Dispatch foreground | Every fenced block in a shipped skill or agent file that carries a `subagent_type:` line also carries `run_in_background: false`. A dispatch is one Agent tool call and the call is the wait: a background dispatch lets the dispatcher end its turn with work still in flight, and a turn that ends in a headless session is a process that exits and takes the work with it. Blocks naming no subagent type are untouched, so a report template, a ledger shape or a shell snippet is not in scope. A fence left open at end of file is refused on its own line rather than skipped, so the block it swallowed cannot read as a clean file with nothing to check. Runs its own helper over a clean fixture, one whose template leaves the parameter off, and one whose last fence never closes, so the refusal is shown rather than assumed. Cannot judge whether a running agent actually passes the parameter; it proves the shipped text tells it to. |
