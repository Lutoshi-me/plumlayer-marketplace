...
| 11 | Pass-knowledge excerpt | The shipped `scripts/cut_pass_knowledge.py`, imported in-process and run over every trade file `MANIFEST.md` lists, produces an excerpt in which each trade's whole `## 3. Scope grain rules` section, and each headed structural gap list block, appears as a contiguous byte-identical run. Contiguity is the assertion, not line presence: a set-of-lines check would pass on text that had been reordered or reflowed, which is what a verbatim cut exists to rule out. The excerpt's knowledge version is compared against the version this check parses out of `MANIFEST.md` itself, so a hardcoded or stale pin fails. The section-7 arm is asserted in both directions: section 7 is present for every trade whose section 3 does not carry the phrase `structural gap list`, and absent for every trade whose section 3 does. Honest bound in the detail line: four trade files (`abatement`, `demo`, `drywall`, `earthwork`) carry no structural gap list anywhere in the source, so no assertion here can prove the excerpt carries one for them; the check names them rather than passing over them silently. |
