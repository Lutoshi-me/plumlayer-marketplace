...
| 11 | Pass-knowledge excerpt | The shipped `scripts/cut_pass_knowledge.py`, imported in-process and run over every trade file `MANIFEST.md` lists, produces an excerpt in which each trade's whole `## 3. Scope grain rules` section, and each headed structural gap list block, appears as a contiguous byte-identical run. Contiguity is the assertion, not line presence: a set-of-lines check would pass on text that had been reordered or reflowed, which is what a verbatim cut exists to rule out. The excerpt's knowledge version is compared against the version this check parses out of `MANIFEST.md` itself, so a hardcoded or stale pin fails. Every manifest-listed trade file must either carry the `### The structural gap list` heading inside its section 3, or state in section 8 that it carries no structural gap list; a file with neither is an error naming the file. The detail line names the files in the second class (today `abatement`, `demo`, `drywall`, `earthwork`), whose gap list a later corpus round writes; nothing here can prove the excerpt carries one for them. |
