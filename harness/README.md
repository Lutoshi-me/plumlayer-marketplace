...
| 12 | Plan inventory | The shipped `scripts/plan_inventory.py`, imported in-process and run end to end over an invented fixture grid: 30 sheets, three disciplines, one untyped, one 27-sheet pass. Asserts the inventory mode's per-discipline counts and its discipline-by-sheet-type cross tab against a tally this check computes itself off the fixture; that `--expect-count` off by one refuses in one line; that the expand mode's unit lines carry exactly the expected sheets, once each, in grid order, each citing the page the fixture gives it; that the 27-unit pass splits into legs `A1a`, `A1b`, `A1c` sized 9, 9, 9; and that three deliberately broken pass assignments (one sheet in two passes, a sheet in no pass and no exclusion, a pass carrying eleven trade files) each exit 1 with a single stderr line. Both modes' bounds lines are asserted to name their counts, so a run that reports nothing fails. Honest bound in the detail line: the fixture is invented and small, so this proves the script's arithmetic and its refusals, and nothing about how a real grid file's fields arrive. |
