...
| 10 | Runner mode set | The `##` headings of `agents/scope-round-runner.md` match the pinned set exactly, in both directions (`What your dispatch gives you`, `Pass mode`, `The ledger lines`, `Boundary mode`, `Completeness mode`, `What you never do`, `Your summary`). This is the cheap mechanical way to catch the per-pass shape being partly undone: a `## Round mode` coming back, or `## Pass mode` renamed away, fails the release. Follows the `EXPECTED_SKILLS` / `EXPECTED_AGENTS` precedent, which are already asserted both ways. |
| 11 | Pass-knowledge excerpt | The shipped `scripts/cut_pass_knowledge.py`, imported in-process and run over every trade `MANIFEST.md` lists, produces an excerpt in which each trade's whole `hints/<slug>.md` appears as a contiguous byte-identical run. Contiguity is the assertion, not line presence: a set-of-lines check would pass on text that had been reordered or reflowed, which is what a verbatim cut exists to rule out. The excerpt's knowledge version is compared against the version this check parses out of `MANIFEST.md` itself, so a hardcoded or stale pin fails. Every manifest-listed trade must ship both `hints/<slug>.md` and `conventions/<slug>.md`; every hints file is checked against its shape and its two budgets (twenty hint lines, 2,400 characters) recomputed here rather than read off the script, so a drift between the source repo's Node gate and this Python one is a release failure and not a silent disagreement; every convention table is checked to be a title line plus one table with the four columns in order and no name cell over eighty characters. The join the runner depends on is asserted too: the cut is run over every catalog code `trade-sheets.json` maps, and every slug it prints must have a convention table on disk, because the cut is the only place a code becomes a slug and the runner builds its conventions path from what the cut printed. Four broken invocations (a hints file over each budget, a manifest trade with no hints file, one with no conventions file) each refuse with exit 1 and one stderr line naming what is wrong. Honest bound: shape and budget only. Nothing here can judge whether a hint earns its line or a row is a real convention. |
| 12 | Plan inventory | The shipped `scripts/plan_inventory.py`, imported in-process and run end to end over an invented fixture set: a 31-sheet grid across six disciplines (one with no discipline, one with no sheet type, every vocabulary type present, one trade whose sheets exceed a pass), a packages fixture, a definitions fixture in the shapes `list_definition_kinds` and `list_definitions` return, and a citation index fixture of `index_citations_leftover` pages with and without located codes. Asserts the inventory mode's counts against a tally the check computes itself; that `--expect-count` off by one refuses in one line; that window 1 selects exactly the vocabulary sheets plus includes minus excludes, once each, every unit line citing the page the grid gives it; that window 2 writes one pass per package, splits the over-twelve trade into balanced lettered passes, runs each seam group of trades as one contiguous block and names a group no line order can satisfy, resolves a package's code to its trade file by nearest CSI ancestor and counts those, and reports packages with no sheet family and families that named no sheet; that window 3 is exactly the inventory minus window 2's own unit lines and reports the index's open entries; that every partial input reaches the bounds line by name (a code row with no defining sheet, a kind short of its own count, a located row naming no kind, a leftover kind short of its own total, and an index with no located codes at all); and that eleven broken invocations each refuse with exit 1 and one stderr line naming what is missing, a leftover page read off a pass that has not finished among them. Every window's bounds line must name its counts. Honest bound: the fixtures are invented and small. They carry the field names the shipped verbs return, so a rename on the record's side fails here, but nothing about how a real grid, definitions read or leftover read arrives is proved by them. |
| 13 | No fork subagent | No shipped skill or agent file names `fork` as a subagent type, in either the `subagent_type:` dispatch-line shape this codebase's own templates use, or a `tools: Agent(fork)` frontmatter declaration. Regression guard for a runner that invented a wait primitive by dispatching fork agents (one told to wait for a reader's completion, two told to do nothing and return done) instead of making the reader dispatch itself the wait. Ordinary English uses of "fork" (a forklift, a decision fork) are untouched: the pattern only matches "fork" sitting immediately after one of the two anchors, and `trade-knowledge/` is outside this check's file scope regardless. |
| 14 | Trade sheet map | `trade-knowledge/trade-sheets.json` maps every manifest trade file to a catalog trade id and the sheet families that trade reads, in both directions; its pinned sheet-type vocabulary equals the recognizer's; every seam pair names two mapped trades; a trade with no family carries a reason; the one named exception (a trade with no catalog match) is allowed only with its reason. The file also sits under the banned-string and retired-vocabulary scans. |
...
`description`, the shipped agent set matches the expected two (`scope-reader`,
`scope-round-runner`) in both directions, and no agent declares `hooks`,
