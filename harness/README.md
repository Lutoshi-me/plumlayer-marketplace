...
| 5 | Estimator-block identical | The `## Talk to your user like an estimator` block, in every skill that carries it, is byte-identical against the single canonical copy in `docs/plugin-text-style.md` §3 (not compared skill to skill, since each skill's own unheaded intro paragraph follows the block before the next heading). Fails loudly if the canonical block can't be parsed out of the style doc. On a mismatch, names the file and the first differing line. |
| 6 | Banned strings | No client-name denylist hit, `PLU-\d+`, internal vault filename, `MOSOT`, em dash, or middle dot in shipped text. The full set applies to skills, the root README, all four manifest JSON files, and anything in `trade-packages/` that isn't a pinned corpus entry (currently `MANIFEST.md`, hand-authored release prose, and any future hand-authored file dropped in beside the entries); the client-name denylist alone applies to the pinned entries themselves, read from `MANIFEST.md`'s own "## Entries" list rather than hardcoded, so a file not on that list defaults to the full scan. Em dash and middle dot are exempt inside fenced code blocks and inline code spans, since those are data (e.g. the citation format, the claim-atom notation), not prose; every other pattern still applies inside code. The detail line reports the two scan populations as separate counts. Each hit is reported with file, line, and the offending match. |
