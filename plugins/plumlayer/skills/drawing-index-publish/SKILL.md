...
description: >
  Publish the project's Master Drawing Index Excel workbook off the cloud project record's set grid:
  a Current Set tab, one tab per drawing delivery in order, and a trailing Review tab of sheets that
  still need a look. Trigger on "master drawing index", "publish the drawing index", "drawing index
  workbook", "drawing index xlsx", or "/publish-drawing-index". Drives the async publish_master_index
  and publish_master_index_status MCP tools, a pure export that records nothing new. Does not
  assemble the PDFs (drawing-set-assemble) or upload drawings (drawing-upload).
---
...
since the two use different job slots, so one may run while the other is active. Each fresh run mints a new
