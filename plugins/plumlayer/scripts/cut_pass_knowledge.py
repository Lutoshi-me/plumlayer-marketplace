...
script writes one pass knowledge file holding just the section the reader's mandates act on,
copied byte for byte out of the shipped trade files, with the knowledge version at the top.

The cut is deterministic and the section is verbatim because a paraphrase would be an unrecorded
rewrite of knowledge that every convention-line record cites by version.

Sections carried, per trade:
  - `## 3. Scope grain rules`, always, whole. Every trade file that carries a structural gap list
    carries it there, so section 3 alone holds both mandates the reader acts on.
...
def cut(trade_knowledge: Path, trades: list[str], pass_id: str, out: Path) -> str:
    ...
    for slug in slugs:
        ...
        # The bound is read off section 3 alone, because section 3 is all the excerpt carries: a
        # gap list anywhere else in the trade file never reaches the reader.
    ...
    header = (
        ...
        "sections carried, verbatim and whole: 3. Scope grain rules.\n"
        "sections not carried: 1, 2, 4, 5, 6, 7, 8. This is an excerpt, not the trade file. Where a\n"
    ...
    return (
        ...
        f"sections carried: 3 (all); "
        f"no gap list in section 3 for: {gapless_names}; resolved from a trade id: {catalog_names}"
    )
