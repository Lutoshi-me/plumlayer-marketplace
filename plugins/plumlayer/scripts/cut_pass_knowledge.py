...
Exit codes:
  ...
  1  a named failure, one line on stderr: the manifest is unreadable or carries no knowledge
     version or no trade list; a trade slug is not in the manifest's list; a named trade file is
     missing; a trade file has no section 3; the output path cannot be written.
...
def cut(trade_knowledge: Path, trades: list[str], pass_id: str, out: Path) -> str:
    ...
    excerpt is a projection off the shipped files, and a later leg of the same pass that found a
    ...
    slugs = _dedupe(trades)
    unknown = [s for s in slugs if s not in known]
    if unknown:
        raise CutError(
            f"not in {manifest_path.name}'s trade list: {', '.join(unknown)}"
        )
    ...
    return (
        ...
        f"no gap list found in: {gapless_names}"
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="trade slugs the pass carries, comma separated",
    )
    parser.add_argument("--pass-id", required=True, help="the pass or leg id")
